...
function sessionKey(landRecordId: string) {
  return `${CONV_KEY_PREFIX}:${landRecordId}`;
}

function getOrCreateConversationId(landRecordId: string): string {
  const key = sessionKey(landRecordId);
  try {
...
function toApiHistory(messages: UiMessage[]): ApiChatMessage[] {
...
export function ChatSidebar({
  ...
  const [conversationId, setConversationId] = useState<string>(() =>
    getOrCreateConversationId(landRecordId)
  );
  ...
  useEffect(() => {
    if (prevLandRef.current !== landRecordId) {
      ...
      setConversationId(getOrCreateConversationId(landRecordId));
  ...
  const send = useCallback(async () => {
    ...
    try {
      const res = await chatQuery({
        ...
        chat_history: toApiHistory(priorForApi),
      });

      if (res.conversation_id) {
        setConversationId(res.conversation_id);
        try {
          sessionStorage.setItem(sessionKey(landRecordId), res.conversation_id);
        } catch {
          /* ignore */
        }
      }
  ...
  }, [input, loading, conversationId, landRecordId]);
                  ...
                  {messages.map((m, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {isUser ? 'You' : CHAT_ASSISTANT_NAME}
                        </span>
                        <div
                        ...
                        >
                          ...
                          {!isUser &&
                            m.sources?.map((src, si) => {
                              const quote =
                                src.excerpt?.trim() ||
                                `${src.document_name} (p. ${src.page_numbers?.join(', ') ?? '—'})`;
                              return (
                                <p
                                  key={si}
